...
def autoregressive_log_prob(
    ...
    action_history = torch.zeros(B, K, 3, device=device)
    log_probs = []
    for k in range(K):
        ...
        action_history[:, k, :] = a_k.detach()
        # Clamp for next embedding
        action_history[:, k, 0] = a_k[:, 0].clamp(0, num_shards - 1)
        action_history[:, k, 1] = a_k[:, 1].clamp(0, num_shards - 1)
        action_history[:, k, 2] = a_k[:, 2].clamp(0, num_prefixes - 1)
...
def ppo_update(
    ...
    for _ in range(config.epochs_per_update):
        for sb_st, sb_ah, sb_act, sb_old_lp, sb_ret, sb_adv in loader:
            ...
            _, _, new_entropy, _ = net.get_action_and_value(sb_st, sb_ah, action=sb_act[:, 0, :])
            ...
            loss = policy_loss + config.value_coef * value_loss - config.entropy_coef * new_entropy.mean()
            optimizer.zero_grad()
            loss.backward()
            ...
            total_entropy += new_entropy.mean().item()
            n_batches += 1
    return {

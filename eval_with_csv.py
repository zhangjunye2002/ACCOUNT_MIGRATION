...
流程（不再用仿真环境，而是真实 CSV）：
1. 按块流式读 CSV（data.stream_epochs）；
...
def compute_u_v(state) -> tuple[float, float]:
    ...
    - v_t: CST 方差的负值
    """
    from aero.reward import compute_reward
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="每个 epoch 包含的块数量（和 AERO 论文设定类似）",
    )
    parser.add_argument(
        "--max-epochs",
        type=int,
    ...
    parser.add_argument(
        ...
        help="评估结果输出 CSV 路径；设为空字符串可关闭落盘",
    )
    ...
    print(
        ...
        f"blocks_per_epoch={args.blocks_per_epoch}"
    )
    ...
    for batch in stream_epochs(csv_path, blocks_per_epoch=args.blocks_per_epoch):
        ...
        print(
            f"[epoch {epoch_idx}] blocks [{batch.block_start},{batch.block_end}], "
        ...
        if args.mode == "drl":
            ...
            for m in plan:
                if (
                    0 <= m.prefix < P
                    and 0 <= m.sender_shard < N
                    and 0 <= m.receiver_shard < N
                ):
                    # 只有当当前映射确实在 sender_shard 时，才视为有效迁移
                    if (
                        prefix_to_shard[m.prefix] == m.sender_shard
                        and m.sender_shard != m.receiver_shard
                    ):
                        prefix_to_shard[m.prefix] = m.receiver_shard
                        applied += 1

            print(f"         migrations proposed={proposed}, applied={applied}")
        else:
